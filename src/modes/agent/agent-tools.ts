import { tool } from "ai";
import { z } from "zod";
import type { ToolExecutor } from "../../shared/execution/tool-executor";
...
export const agentTools = (executor: ToolExecutor) => {
  return {
    ...
    // Creates a new file and stages it for approval.
    create_file: tool({
    ...
    // Replaces the contents of an existing file and stages it for approval.
    modify_file: tool({
      description:
    ...
    // Deletes a file and stages the action for approval.
    delete_file: tool({
    ...
    // Creates a folder structure and stages it for approval.
    create_folder: tool({
      description:
    ...
    // Runs a shell command after the user approves it.
    execute_shell: tool({
      description:
