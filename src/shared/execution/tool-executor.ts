...
// checks whether given file is in our required format or not
...
export class ToolExecutor {
  ...
  // Cached skill roots directory list
  private cachedSkillRoots: string[] | null = null;
  ...
  skillRoots(): string[] {
    if (this.cachedSkillRoots) return this.cachedSkillRoots;
    const extra =
      process.env.SKILLS_DIRS?.split(/[;]/)
        .map((s) => s.trim())
        .filter(Boolean) ?? [];
    this.cachedSkillRoots = [
      ...extra,
      path.join(homedir(), ".cursor/skills-cursor"),
      path.join(homedir(), ".claude/skills"),
    ];
    return this.cachedSkillRoots;
  }
  ...
  applyApprovedFromTracker(): { errors: string[] } {
    ...
    for (const a of all.filter(
      ...
      if (r.error) {
        errors.push(`shell execution failed: ${getErrorMessage(r.error)}: ${cmd}`);
